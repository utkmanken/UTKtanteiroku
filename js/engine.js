...
const BACKLOG_HELPERS = (() => {
	...
	const fallbackIconPath = "material/mainmenu_icon/chara_icon_other.jpg";
	...
	const getCharaIconUrl = (charaId, cachedCharaImageIconKeys = getCharaImageIconKeys()) => {
		const iconKeys = [];
		const fallbackIconUrl = () => ASSETS.imgs[fallbackIconPath] ?? errorImageUrl;
		...
		for (const iconKey of iconKeys) {
			...
			if (iconUrl && iconUrl !== errorImageUrl) return iconUrl;
		}
...
async function first_display_change() {
	...
	const ending_story = async () => {
		...
		if (SCENARIO[last_sc_line_inSummary] === "end") {
			isAutoMode = false;
			alert("タイトル画面へ戻ります");
			await first_display_change();
		}
...
async function display_change_toStory() {
		...
		document.getElementById("mainmenubutton").addEventListener('click', async () => {
			...
			const skip_logic = async () => {
				const original_current_display = current_display;
				current_display = "continue";
				isInRSMEbutCONTINUE = true;
				if (original_current_display === "record_summary" || original_current_display === "method" || original_current_display === "ending") return; //万が一の防衛
				if (!window.confirm("ストーリーをスキップしますか")) return;
				skipButton.disabled = true;
				const current_sc = original_current_display !== "record_summary" && original_current_display !== "method" && original_current_display !== "ending" ? last_sc_line : last_sc_line_inSummary;
				...
				for (let i = current_sc + 1; i < next_stop; i++) {
					let returned_index = await do_scenario(i);
					i = returned_index;
					if (original_current_display !== "record_summary" && original_current_display !== "method" && original_current_display !== "ending") {
						last_sc_line = i;
						localStorage.setItem("last_sc_line", String(last_sc_line));
					} else {
						last_sc_line_inSummary = i;
				...
				do_inaninstant = false;
				current_display = original_current_display;
				isInRSMEbutCONTINUE = false;
				closemenu();
				await do_scenario(next_stop);//qr等はそのコマンドがすぐにreturnを返さないと予想: QRを読み込んだりしなきゃいけないから
				if (original_current_display !== "record_summary" && original_current_display !== "method" && original_current_display !== "ending") {
		...
		const next_button_func = async (e) => {
			...
			if (current_display !== "record_summary" && current_display !== "method" && current_display !== "ending") {
				...
				if (last_sc_line_inSummary+1 < record_summary_do_scenario[record_playback_num][1]) {
					...
					if (SCENARIO[last_sc_line_inSummary] === "end") {
						isAutoMode = false;
						if (current_display === "record_summary") await return_to_record();
						else {
							alert("タイトル画面へ戻ります");
							await first_display_change();
						}
		...
		*/
		...
		async function automode() {
			const index_in_automode = current_display !== "record_summary" && current_display !== "method" && current_display !== "ending" && !isInRSMEbutCONTINUE ? last_sc_line + 1 : last_sc_line_inSummary + 1;
			const functionname = SCENARIO[index_in_automode] ?? "end";
			const CD_check = ["record_summary", "method", "ending"];
			const current_funcname = CD_check.includes(current_display) || isInRSMEbutCONTINUE ? SCENARIO[last_sc_line_inSummary]??"end" : SCENARIO[last_sc_line]??"end";
			if (automode_stop_command.includes(current_funcname)) {
				off_automode();
			}
