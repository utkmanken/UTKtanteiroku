...
const QR_CAMERA_ERROR_RESULT = "qr_camera_error";
const QR_CAMERA_ERROR_MESSAGE = "カメラ使用に失敗しました。再読み込みしても直らない場合は漫研部員までお知らせください";

const qr_logic = async () => {
	return new Promise((resolveLogic) => {
		if (typeof Html5Qrcode === "undefined") {
			console.warn("qr: html5-qrcodeが読み込まれていません。");
			resolveLogic(QR_CAMERA_ERROR_RESULT);
			return;
		}
		...
		const cleanupAndResolve = async (result) => {
			...
			if (cameraMonitorTimer !== null) {
				clearInterval(cameraMonitorTimer);
		...
		const resolveCameraError = (message, error = null) => {
			if (error) {
				console.warn(message, error);
			} else {
				console.warn(message);
			}
			cleanupAndResolve(QR_CAMERA_ERROR_RESULT);
		};
		const startCameraAvailabilityMonitor = () => {
			...
			if (video) {
				video.addEventListener("error", () => {
					resolveCameraError("qr: カメラ映像でエラーが発生しました。", video.error);
				}, { once: true });
			}
			getVideoTracks().forEach((track) => {
				track.addEventListener("ended", () => {
					resolveCameraError("qr: カメラが途中で利用できなくなりました。");
				}, { once: true });
			});
			if (cameraMonitorTimer !== null) return;
		...
		Html5Qrcode.getCameras()
			.then((cameras) => {
				...
				return html5QrCode.start(cameraId, cameraConfig, (decodedText) => {
					...
					if (isFinished) return;
					isRunning = true;
					startCameraAvailabilityMonitor();
	...
	qr: async (args) => {
		...
		return new Promise((resolve) => {
			...
			const finishQrRead = (qrRead) => {
				...
				if (qrRead === QR_CAMERA_ERROR_RESULT) {
					closeQrCameraDisplay();
					window.alert(QR_CAMERA_ERROR_MESSAGE);
					return;
				}
			...
			routeButton.addEventListener('click', async (event) => {
				...
				if (camera_status !== "granted") {
					console.warn("qr: カメラが許可されていないため、カメラ読み取りを開始できません。");
					qrRead = QR_CAMERA_ERROR_RESULT;
				} else {
